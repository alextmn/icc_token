...
/**
 * The public key of the account we are dealing with
 */
...
/**
 * Check if the ICC token BPF program has been deployed
 */
 ...
 */
export async function makeICCTransfer(icc_payload: Buffer): Promise<string> {
  console.log('Making ICC transfer', greetedPubkey.toBase58());
  const instruction = new TransactionInstruction({
    keys: [{pubkey: greetedPubkey, isSigner: false, isWritable: true}],
    programId,
    data: icc_payload, // All instructions are hellos
  });
  const str = await sendAndConfirmTransaction(
    connection,
...
/**
 * Report the number of times the greeted account has been said transfer
 */
export async function reportTransfers(): Promise<void> {
  const accountInfo = await connection.getAccountInfo(greetedPubkey);
  if (accountInfo === null) {
    throw 'Error: cannot find the transfered account';
  }
